...
{
	public class Homepage_SROEst : IHomepage_SROEst
	{
		...
		public Homepage_SROEst(IApplicationDB appDB,
		...
		public (ICollectionLoadResponse Data, int? ReturnCode) Homepage_SROEstSp(int? DaysBefore = 30)
		{
			...
			AmountType _TotalPrice = DBNull.Value;
			DateTime? CutoffDate = null;

			if (existsChecker.Exists(
				tableName: "optional_module",
				fromClause: collectionLoadRequestFactory.Clause("AS [om]"),
				whereClause: collectionLoadRequestFactory.Clause("ISNULL([om].[is_enabled], 0) = 1 AND OBJECT_ID(QUOTENAME('Homepage_SROEstSp' + CHAR(95) + [om].[ModuleName])) IS NOT NULL")))
			{
				//BEGIN
				...
				{
					optional_module1Item.SetValue<string>("SpName", stringUtil.QuoteName("Homepage_SROEstSp" + stringUtil.Char(95) + optional_module1Item.GetValue<string>("u0")));
				};
					...
					whereClause: collectionLoadRequestFactory.Clause("")))
				{
					...
					var ALTGEN = AltExtGen_Homepage_SROEstSp(_ALTGEN_SpName, DaysBefore);
			...
			if (this.scalarFunction.Execute<int?>("OBJECT_ID", "dbo.EXTGEN_Homepage_SROEstSp") != null)
			{
				var EXTGEN = AltExtGen_Homepage_SROEstSp("dbo.EXTGEN_Homepage_SROEstSp",
				DaysBefore);
			...
			var fs_sroLoadRequest = collectionLoadRequestFactory.SQLLoad(columnExpressionByVariableToAssign: new Dictionary<IUDDTType, string>()
				{
					{_TotalPrice,"ISNULL(SUM(dbo.SSSFSSroGetDerValue(sro_num, 'DerTotalPrice')), 0)"},
				},
				loadForChange: false, 
                lockingType: LockingType.None,
                tableName: "fs_sro",
				fromClause: collectionLoadRequestFactory.Clause(""),
				whereClause: collectionLoadRequestFactory.Clause("sro_stat = 'E' AND start_date >= {0}", CutoffDate),
				orderByClause: collectionLoadRequestFactory.Clause(""));
			...
			var nonTableLoadRequest = collectionLoadBuilderRequestFactory.Create(columnExpressionByColumnName: new Dictionary<string, object>()
				{
					...
					{ "Amount", variableUtil.GetValue<AmountType>(_TotalPrice,true)},
				});
		...
		public (ICollectionLoadResponse Data, int? ReturnCode) AltExtGen_Homepage_SROEstSp(string AltExtGenSp, int? DaysBefore = 30)
		{
