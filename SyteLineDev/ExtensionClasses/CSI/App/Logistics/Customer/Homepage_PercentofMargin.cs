...
//CLASS NAME: Homepage_PercentofMargin.cs
...
{
	public class Homepage_PercentofMargin : IHomepage_PercentofMargin
	{
		...
		public Homepage_PercentofMargin(IApplicationDB appDB,
		...
		public (ICollectionLoadResponse Data, int? ReturnCode) Homepage_PercentofMarginSp (int? DaysBefore = 30)
		{
			...
			TolerancePercentType _AvgPct = DBNull.Value;
			DateTime? CutoffDate = null;

			if (existsChecker.Exists(
				tableName:"optional_module",
				fromClause: collectionLoadRequestFactory.Clause("AS [om]"),
				whereClause: collectionLoadRequestFactory.Clause("ISNULL([om].[is_enabled], 0) = 1 AND OBJECT_ID(QUOTENAME('Homepage_PercentofMarginSp' + CHAR(95) + [om].[ModuleName])) IS NOT NULL")))
			{
				//BEGIN
				...
				foreach(var optional_module1Item in optional_module1LoadResponse.Items){
					optional_module1Item.SetValue<string>("SpName", stringUtil.QuoteName("Homepage_PercentofMarginSp" + stringUtil.Char(95) + optional_module1Item.GetValue<string>("u0")));
				};
					...
					whereClause: collectionLoadRequestFactory.Clause("")))
				{
					...
					var ALTGEN = AltExtGen_Homepage_PercentofMarginSp (_ALTGEN_SpName, DaysBefore);
			...
			if (this.scalarFunction.Execute<int?>("OBJECT_ID", "dbo.EXTGEN_Homepage_PercentofMarginSp") != null)
			{
				var EXTGEN = AltExtGen_Homepage_PercentofMarginSp("dbo.EXTGEN_Homepage_PercentofMarginSp", DaysBefore);
			...
			var fs_sroLoadRequest = collectionLoadRequestFactory.SQLLoad(columnExpressionByVariableToAssign: new Dictionary<IUDDTType, string>()
				{
					{_AvgPct,"AVG(CASE WHEN total_price = 0 THEN 0 ELSE (total_price - (total_cost_lbr + total_cost_matl + total_cost_misc)) / total_price * 100 END)"},
				},
				loadForChange: false, 
                lockingType: LockingType.None,
                tableName:"fs_sro",
			...
			var nonTableLoadRequest = collectionLoadBuilderRequestFactory.Create(columnExpressionByColumnName: new Dictionary<string, object>()
				{
					...
					{ "Percentage", variableUtil.GetValue<TolerancePercentType>(_AvgPct,true)},
				});
		...
		public (ICollectionLoadResponse Data, int? ReturnCode) AltExtGen_Homepage_PercentofMarginSp(string AltExtGenSp, int? DaysBefore = 30)
		{
