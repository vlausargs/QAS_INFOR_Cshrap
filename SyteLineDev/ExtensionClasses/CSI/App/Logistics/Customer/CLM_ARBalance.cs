...
{
	public class CLM_ARBalance : ICLM_ARBalance
	{
		...
		readonly IARBalanceByPeriod iARBalanceByPeriod;
		...
		public CLM_ARBalance(IApplicationDB appDB,
			...
			IARBalanceByPeriod iARBalanceByPeriod,
			...
			ISQLValueComparerUtil sQLUtil)
		{
			...
			this.iARBalanceByPeriod = iARBalanceByPeriod;
			...
			string Infobar = null)
		{
			...
			try
			{
				...
				if (existsChecker.Exists(tableName:"optional_module",
					...
					whereClause: collectionLoadRequestFactory.Clause("ISNULL([om].[is_enabled], 0) = 1 AND OBJECT_ID(QUOTENAME('CLM_ARBalanceSp' + CHAR(95) + [om].[ModuleName])) IS NOT NULL"))
				)
				{
					//BEGIN
					...
					foreach(var optional_module1Item in optional_module1LoadResponse.Items){
						optional_module1Item.SetValue<string>("SpName", stringUtil.QuoteName(stringUtil.Concat("CLM_ARBalanceSp", stringUtil.Char(95), optional_module1Item.GetValue<string>("u0"))));
					};
					...
					)
					{
						...
						var ALTGEN = AltExtGen_CLM_ARBalanceSp (ALTGEN_SpName,
							FiscalYear,
							Period,
							SiteGroup,
							FilterString,
				...
				if (this.scalarFunction.Execute<int?>("OBJECT_ID", "dbo.EXTGEN_CLM_ARBalanceSp") != null)
				{
					var EXTGEN = AltExtGen_CLM_ARBalanceSp("dbo.EXTGEN_CLM_ARBalanceSp",
						FiscalYear,
						Period,
						SiteGroup,
						FilterString,
				...
				this.sQLExpressionExecutor.Execute(@"Declare
					...
					SELECT @CustNum AS UbCustNum,
					       @CustName AS UbCustName,
					       @Opening AS UbOpening,
					       @Invoice AS UbInvoice,
					       @Payment AS UbPayment,
					       @Closing AS UbClosing,
					       @CustCurrCode AS CustCurrCode,
				...
				//Please Generate the bounce for this stored procedure:ARBalanceByPeriodSp
				var tempOutput1ExecResult = this.iARBalanceByPeriod.ARBalanceByPeriodSp(
					FiscalYear: FiscalYear,
					Period: Period,
					SiteGroup: SiteGroup,
					Infobar: Infobar);
				var tempOutput1LoadResponse = collectionLoadResponseUtil.CloneCollectionRenameColumns(tempOutput1ExecResult.Data,
					new List<string>() { "UbCustNum",
							"UbCustName",
							"UbOpening",
							"UbInvoice",
							"UbPayment",
				...
				PropertyList = "UbDispOpening;UbDispInvoice;UbDispPayment;UbDispClosing;CadCurrCode";
				ColumnList = "UbOpening;UbInvoice;UbPayment;UbClosing;CustCurrCode";
				...
				WhereClause = null;
				AdditionalClause = "ORDER BY UbCustNum";
				KeyColumns = "UbCustNum";
