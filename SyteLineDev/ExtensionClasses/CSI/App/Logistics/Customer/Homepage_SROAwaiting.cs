...
//CLASS NAME: Homepage_SROAwaiting.cs
...
{
	public class Homepage_SROAwaiting : IHomepage_SROAwaiting
	{
		...
		public Homepage_SROAwaiting(IApplicationDB appDB,
		...
		public (ICollectionLoadResponse Data, int? ReturnCode) Homepage_SROAwaitingSp (int? DaysBefore = 30)
		{
			...
			IntType _TotalCount = DBNull.Value;
			DateTime? CutoffDate = null;

			if (existsChecker.Exists(
				tableName:"optional_module",
				fromClause: collectionLoadRequestFactory.Clause("AS [om]"),
				whereClause: collectionLoadRequestFactory.Clause($"ISNULL([om].[is_enabled], 0) = 1 AND OBJECT_ID(QUOTENAME('Homepage_SROAwaitingSp' + CHAR(95) + [om].[ModuleName])) IS NOT NULL")))
			{
				//BEGIN
				...
				foreach(var optional_module1Item in optional_module1LoadResponse.Items){
					optional_module1Item.SetValue<string>("SpName", stringUtil.QuoteName("Homepage_SROAwaitingSp" + stringUtil.Char(95) + optional_module1Item.GetValue<string>("u0")));
				};
					...
					whereClause: collectionLoadRequestFactory.Clause("")))
				{
					...
					var ALTGEN = AltExtGen_Homepage_SROAwaitingSp (_ALTGEN_SpName, DaysBefore);
			...
			if (this.scalarFunction.Execute<int?>("OBJECT_ID", "dbo.EXTGEN_Homepage_SROAwaitingSp") != null)
			{
				var EXTGEN = AltExtGen_Homepage_SROAwaitingSp("dbo.EXTGEN_Homepage_SROAwaitingSp", DaysBefore);
			...
			var fs_sroLoadRequest = collectionLoadRequestFactory.SQLLoad(columnExpressionByVariableToAssign: new Dictionary<IUDDTType, string>()
                ...
                tableName:"fs_sro",
				fromClause: collectionLoadRequestFactory.Clause(""),
				whereClause: collectionLoadRequestFactory.Clause("awaiting_parts = 1 AND start_date >= {0}",CutoffDate),
				orderByClause: collectionLoadRequestFactory.Clause(""));
			...
			var nonTableLoadRequest = collectionLoadBuilderRequestFactory.Create(columnExpressionByColumnName: new Dictionary<string, object>()
				{
					...
					{ "Count", variableUtil.GetValue<IntType>(_TotalCount,true)},
				});
		...
		public (ICollectionLoadResponse Data, int? ReturnCode) AltExtGen_Homepage_SROAwaitingSp(string AltExtGenSp, int? DaysBefore = 30)
		{
