...
//CLASS NAME: Homepage_LateSROCount.cs
...
{
	public class Homepage_LateSROCount : IHomepage_LateSROCount
	{
		...
		public Homepage_LateSROCount(IApplicationDB appDB,
		...
		public (ICollectionLoadResponse Data, int? ReturnCode) Homepage_LateSROCountSp (int? DaysBefore = 30)
		{
			...
			if (existsChecker.Exists(
				...
				whereClause: collectionLoadRequestFactory.Clause("ISNULL([om].[is_enabled], 0) = 1 AND OBJECT_ID(QUOTENAME('Homepage_LateSROCountSp' + CHAR(95) + [om].[ModuleName])) IS NOT NULL")))
			{
				//BEGIN
				...
				foreach(var optional_module1Item in optional_module1LoadResponse.Items){
					optional_module1Item.SetValue<string>("SpName", stringUtil.QuoteName("Homepage_LateSROCountSp" + stringUtil.Char(95) + optional_module1Item.GetValue<string>("u0")));
				};
					...
					whereClause: collectionLoadRequestFactory.Clause("")))
				{
					...
					var ALTGEN = AltExtGen_Homepage_LateSROCountSp (_ALTGEN_SpName, DaysBefore);
			...
			if (this.scalarFunction.Execute<int?>("OBJECT_ID", "dbo.EXTGEN_Homepage_LateSROCountSp") != null)
			{
				var EXTGEN = AltExtGen_Homepage_LateSROCountSp("dbo.EXTGEN_Homepage_LateSROCountSp", DaysBefore);
			...
			var fs_sroLoadRequest = collectionLoadRequestFactory.SQLLoad(columnExpressionByVariableToAssign: new Dictionary<IUDDTType, string>()
                ...
                tableName:"fs_sro",
				fromClause: collectionLoadRequestFactory.Clause(""),
				whereClause: collectionLoadRequestFactory.Clause("sro_stat = 'O' AND open_date >= {0} AND (end_date IS NOT NULL AND end_date < {1})",CutoffDate,Today),
				orderByClause: collectionLoadRequestFactory.Clause(""));
			...
			var nonTableLoadRequest = collectionLoadBuilderRequestFactory.Create(columnExpressionByColumnName: new Dictionary<string, object>()
				{
					...
					{ "Count", variableUtil.GetValue<IntType>(_SROCount,true)},
				});
		...
		public (ICollectionLoadResponse Data, int? ReturnCode) AltExtGen_Homepage_LateSROCountSp(string AltExtGenSp, int? DaysBefore = 30)
		{
