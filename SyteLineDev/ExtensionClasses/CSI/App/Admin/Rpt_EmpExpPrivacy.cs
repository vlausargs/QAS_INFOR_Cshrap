//PROJECT NAME: Admin
//CLASS NAME: Rpt_EmpExpPrivacy.cs
...
namespace CSI.Admin
{
	public class Rpt_EmpExpPrivacy : IRpt_EmpExpPrivacy
	{
		...
		public Rpt_EmpExpPrivacy(IApplicationDB appDB,
			...
			string pSite = null)
		{
			...
			if (existsChecker.Exists(tableName: "optional_module",
				...
				whereClause: collectionLoadRequestFactory.Clause("ISNULL([om].[is_enabled], 0) = 1 AND OBJECT_ID(QUOTENAME('Rpt_EmpExpPrivacySp' + CHAR(95) + [om].[ModuleName])) IS NOT NULL")))
			{
				...
				{
					optional_module1Item.SetValue<string>("SpName", stringUtil.QuoteName(stringUtil.Concat("Rpt_EmpExpPrivacySp", stringUtil.Char(95), optional_module1Item.GetValue<string>("u0"))));
				};
					...
					whereClause: collectionLoadRequestFactory.Clause("")))
				{
					...
					var ALTGEN = AltExtGen_Rpt_EmpExpPrivacySp(_ALTGEN_SpName,
						ApplicantNum,
						EmpNum,
						pSite);
			...
			if (this.scalarFunction.Execute<int?>("OBJECT_ID", "dbo.EXTGEN_Rpt_EmpExpPrivacySp") != null)
			{
				var EXTGEN = AltExtGen_Rpt_EmpExpPrivacySp("dbo.EXTGEN_Rpt_EmpExpPrivacySp",
					ApplicantNum,
					EmpNum,
					pSite);
			...
			var InitSessionContext = this.iInitSessionContext.InitSessionContextSp(
				ContextName: "Rpt_EmpExpPrivacySp",
			...
			#region CRUD LoadToRecord
			var emp_expLoadRequest = collectionLoadRequestFactory.SQLLoad(columnExpressionByColumnName: new Dictionary<string, string>()
				{
					{"employer","employer"},
					{"city","city"},
					{"state","state"},
				...
				orderByClause: collectionLoadRequestFactory.Clause(""));
			var emp_expLoadResponse = this.appDB.Load(emp_expLoadRequest);
			#endregion  LoadToRecord

			Data = emp_expLoadResponse;
