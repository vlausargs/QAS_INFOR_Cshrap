...
//CLASS NAME: SSSVTXPOSGetHeaderInfo.cs
...
{
	public class SSSVTXPOSGetHeaderInfo : ISSSVTXPOSGetHeaderInfo
	{
		readonly IApplicationDB appDB;
		
		public SSSVTXPOSGetHeaderInfo(IApplicationDB appDB)
		{
			this.appDB = appDB;
		}
		...
		public (int? ReturnCode,
			...
			string oCustPo,
			string Infobar,
			string LocationCode) SSSVTXPOSGetHeaderInfoSp(
			string PRefType,
			Guid? pHdrPtr,
			string oVTXInvNo,
			int? oSTJurisCd,
			...
			string LocationCode)
		{
			...
			VTXHdrRef1Type _oVTXInvNo = oVTXInvNo;
			GenericIntType _oSTJurisCd = oSTJurisCd;
			GenericIntType _oSTGeo = oSTGeo;
			...
			GenericIntType _oSFJurisCd = oSFJurisCd;
			GenericIntType _oSFGeo = oSFGeo;
			...
			using (IDbCommand cmd = appDB.CreateCommand())
			{
				...
				cmd.CommandText = "SSSVTXPOSGetHeaderInfoSp";
				...
				return (Severity, oVTXInvNo, oSTJurisCd, oSTGeo, oSTState, oSTCity, oSTZip, oSTCnty, oSTCountry, oSTAddr1, oSTAddr2, oSTAddr3, oSTAddr4, oSFJurisCd, oSFGeo, oSFState, oSFCity, oSFZip, oSFCnty, oSFCountry, oSFAddr1, oSFAddr2, oSFAddr3, oSFAddr4, oCurrCode, oCustNum, oCustSeq, oSlsman, oCustPo, Infobar, LocationCode);
			}
		}
	}
}
