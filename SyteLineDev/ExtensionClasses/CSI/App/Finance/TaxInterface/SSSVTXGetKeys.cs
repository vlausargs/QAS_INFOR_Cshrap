...
{
	public class SSSVTXGetKeys : ISSSVTXGetKeys
	{
		readonly IApplicationDB appDB;
		
		public SSSVTXGetKeys(IApplicationDB appDB)
		{
			this.appDB = appDB;
		}
		...
		public (int? ReturnCode,
			...
			int? oLineRef2,
			int? oLineRef3,
			string Infobar) SSSVTXGetKeysSp(
			string pRefType,
			Guid? pHdrPtr,
			string pLineRefType,
			Guid? pLinePtr,
			...
			string Infobar)
		{
			...
			using (IDbCommand cmd = appDB.CreateCommand())
			{
				...
				cmd.CommandText = "SSSVTXGetKeysSp";
