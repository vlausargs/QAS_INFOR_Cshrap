...
{
	public class BPActual : IBPActual
	{
		readonly IApplicationDB appDB;
		
		public BPActual(IApplicationDB appDB)
		{
			this.appDB = appDB;
		}
		
		public decimal? BPActualFn(
			DateTime? PerStart,
			DateTime? PerEnd,
			string ChartType,
			string Hierarchy,
			...
			string UC4)
		{
			...
			HierarchyType _Hierarchy = Hierarchy;
			AcctType _Acct = Acct;
			...
			using (IDbCommand cmd = appDB.CreateCommand())
			{
				...
				cmd.CommandText = "SELECT  dbo.[BPActual](@PerStart, @PerEnd, @ChartType, @Hierarchy, @Acct, @UC1, @UC2, @UC3, @UC4)";
