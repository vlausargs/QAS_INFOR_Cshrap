...
{
	public class FTReqQty : IFTReqQty
	{
		readonly IApplicationDB appDB;
		
		public FTReqQty(IApplicationDB appDB)
		{
			this.appDB = appDB;
		}
		
		public decimal? FTReqQtyFn(
			decimal? QtyReleased,
			string Units,
			decimal? MatlQty,
			int? Scrap,
			decimal? ScrapFact)
		{
			...
			using (IDbCommand cmd = appDB.CreateCommand())
			{
				...
				cmd.CommandText = "SELECT  dbo.[FTReqQty](@QtyReleased, @Units, @MatlQty, @Scrap, @ScrapFact)";
