//PROJECT NAME: Data
//CLASS NAME: THAInValidApptcStagingRecords.cs
...
{
	public class THAInValidApptcStagingRecords : ITHAInValidApptcStagingRecords
	{
		readonly IApplicationDB appDB;
		
		public THAInValidApptcStagingRecords(IApplicationDB appDB)
		{
			this.appDB = appDB;
		}
		
		public int? THAInValidApptcStagingRecordsFn(
			Guid? PProcessId,
			Guid? RowPointer,
			...
			DateTime? PPayDateEnding)
		{
			...
			using (IDbCommand cmd = appDB.CreateCommand())
			{
				...
				cmd.CommandText = "SELECT  dbo.[THAInValidApptcStagingRecords](@PProcessId, @RowPointer, @PStartingVendNum, @PEndingVendNum, @PStartingVendName, @PEndingVendName, @PSortNameNum, @PPayDateStarting, @PPayDateEnding)";
