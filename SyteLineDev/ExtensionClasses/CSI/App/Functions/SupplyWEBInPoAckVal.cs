//PROJECT NAME: Data
//CLASS NAME: SupplyWEBInPoAckVal.cs

using CSI.Data.SQL.UDDT;
using System;
using System.Data;
using CSI.Data.CRUD;
using CSI.Data.RecordSets;
using CSI.MG;

namespace CSI.Functions
{
	public class SupplyWEBInPoAckVal : ISupplyWEBInPoAckVal
	{
		readonly IApplicationDB appDB;
		
		public SupplyWEBInPoAckVal(IApplicationDB appDB)
		{
			this.appDB = appDB;
		}
		
		public (int? ReturnCode,
			string Infobar) SupplyWEBInPoAckValSp(
			Guid? TmpAckPoRowPointer,
			string Infobar)
		{
			RowPointerType _TmpAckPoRowPointer = TmpAckPoRowPointer;
			Infobar _Infobar = Infobar;
			
			using (IDbCommand cmd = appDB.CreateCommand())
			{
				
				cmd.CommandType = CommandType.StoredProcedure;
				cmd.CommandText = "SupplyWEBInPoAckValSp";
				
				appDB.AddCommandParameter(cmd, "TmpAckPoRowPointer", _TmpAckPoRowPointer, ParameterDirection.Input);
				appDB.AddCommandParameter(cmd, "Infobar", _Infobar, ParameterDirection.InputOutput);
				
				var Severity = appDB.ExecuteNonQuery(cmd);
				
				Infobar = _Infobar;
				
				return (Severity, Infobar);
			}
		}
	}
}
