...
namespace CSI.Codes
{
	public class BuyerValidCRUD : IBuyerValidCRUD
	{
		...
		public BuyerValidCRUD(IApplicationDB appDB,
		...
		public bool Optional_ModuleForExists()
		{
			return existsChecker.Exists(tableName: "optional_module",
				...
				whereClause: collectionLoadRequestFactory.Clause("ISNULL([om].[is_enabled], 0) = 1 AND OBJECT_ID(QUOTENAME('BuyerValidSp' + CHAR(95) + [om].[ModuleName])) IS NOT NULL"));
		}
		...
		public ICollectionLoadResponse SelectOptional_ModuleForInsert()
		{
			...
			{
				optional_module1Item.SetValue<string>("SpName", stringUtil.QuoteName(stringUtil.Concat("BuyerValidSp", stringUtil.Char(95), optional_module1Item.GetValue<string>("u0"))));
			};
		...
		public (Guid? BuyerAvail, int? rowCount) LoadUsernames(string Buyer, Guid? BuyerAvail)
		{
			RowPointerType _BuyerAvail = DBNull.Value;
			...
			var UserNamesLoadRequest = collectionLoadRequestFactory.SQLLoad(columnExpressionByVariableToAssign: new Dictionary<IUDDTType, string>()
				{
					{_BuyerAvail,"UserNames.RowPointer"},
				},
				loadForChange: false,
				lockingType: LockingType.None,
				tableName: "UserNames",
				fromClause: collectionLoadRequestFactory.Clause(""),
				whereClause: collectionLoadRequestFactory.Clause("UserNames.UserName = {0}", Buyer),
				orderByClause: collectionLoadRequestFactory.Clause(""));
			var UserNamesLoadResponse = this.appDB.Load(UserNamesLoadRequest);
			if (UserNamesLoadResponse.Items.Count > 0)
			{
				BuyerAvail = _BuyerAvail;
			}

			int rowCount = UserNamesLoadResponse.Items.Count;
			return (BuyerAvail, rowCount);
		}

		public (int? ReturnCode,
			string Infobar)
		AltExtGen_BuyerValidSp(
			string AltExtGenSp,
			string Buyer,
			string Infobar)
		{
