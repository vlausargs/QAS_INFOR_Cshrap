...
//CLASS NAME: CurrentMaterialsDeleteCRUD.cs
...
{
	public class CurrentMaterialsDeleteCRUD : ICurrentMaterialsDeleteCRUD
	{
		...
		public CurrentMaterialsDeleteCRUD(IApplicationDB appDB,
		...
		public bool Optional_ModuleForExists()
		{
			return existsChecker.Exists(tableName: "optional_module",
				...
				whereClause: collectionLoadRequestFactory.Clause("ISNULL([om].[is_enabled], 0) = 1 AND OBJECT_ID(QUOTENAME('CurrentMaterialsDeleteSp' + CHAR(95) + [om].[ModuleName])) IS NOT NULL"));
		}
		...
		public ICollectionLoadResponse SelectOptional_ModuleForInsert()
		{
			...
			{
				optional_module1Item.SetValue<string>("SpName", stringUtil.QuoteName(stringUtil.Concat("CurrentMaterialsDeleteSp", stringUtil.Char(95), optional_module1Item.GetValue<string>("u0"))));
			};
		...
		public (Guid? ItemPointer, int? rowCount) LoadItemLoad(string ItmItem, Guid? ItemPointer)
		{
			RowPointerType _ItemPointer = DBNull.Value;
			...
			var itemLoadRequest = collectionLoadRequestFactory.SQLLoad(columnExpressionByVariableToAssign: new Dictionary<IUDDTType, string>()
				{
					{_ItemPointer,"item.RowPointer"},
				},
				loadForChange: false,
				lockingType: LockingType.None,
				tableName: "item",
				fromClause: collectionLoadRequestFactory.Clause(" WITH (READUNCOMMITTED)"),
				whereClause: collectionLoadRequestFactory.Clause("item.item = {0}", ItmItem),
				orderByClause: collectionLoadRequestFactory.Clause(""));
			...
			return (ItemPointer, rowCount);
		}

		public (int? ReturnCode,
			string Infobar)
		AltExtGen_CurrentMaterialsDeleteSp(
			string AltExtGenSp,
			string JobType,
			string ItmItem,
			string Infobar)
