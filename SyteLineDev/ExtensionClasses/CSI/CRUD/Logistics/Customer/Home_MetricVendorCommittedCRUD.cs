...
//CLASS NAME: Home_MetricVendorCommittedCRUD.cs
...
{
    public class Home_MetricVendorCommittedCRUD : IHome_MetricVendorCommittedCRUD
    {
        ...
        public Home_MetricVendorCommittedCRUD(IApplicationDB appDB,
        ...
        public bool Optional_ModuleForExists()
        {
            return existsChecker.Exists(tableName: "optional_module",
                ...
                whereClause: collectionLoadRequestFactory.Clause("ISNULL([om].[is_enabled], 0) = 1 AND OBJECT_ID(QUOTENAME('Home_MetricVendorCommittedSp' + CHAR(95) + [om].[ModuleName])) IS NOT NULL"));
        }
        ...
        public ICollectionLoadResponse Optional_Module1Select()
        {
            ...
            {
                optional_module1Item.SetValue<string>("SpName", stringUtil.QuoteName(stringUtil.Concat("Home_MetricVendorCommittedSp", stringUtil.Char(95), optional_module1Item.GetValue<string>("u0"))));
            };
        ...
        public ICollectionLoadResponse Poitem1Select(int? Count)
        {
            var poitem1LoadRequest = collectionLoadStatementRequestFactory.SQLLoad(columnExpressionByColumnName: new Dictionary<string, string>()
                {
                    {"ExtCost","SUM((qty_ordered - qty_received) * item_cost)"},
                    {"po_vend_num","po_vend_num"},
                },
                selectStatement: collectionLoadRequestFactory.Clause(@"SELECT TOP ({0}) @selectList
					FROM poitem
					WHERE poitem.stat = 'O'
					GROUP BY po_vend_num
					ORDER BY ExtCost DESC", Count));

            return this.appDB.Load(poitem1LoadRequest);
        }

        public ICollectionLoadResponse Poitem3Select()
        {
            var poitem3LoadRequest = collectionLoadStatementRequestFactory.SQLLoad(columnExpressionByColumnName: new Dictionary<string, string>()
                {
                    {"ExtCost","SUM((qty_ordered - qty_received) * item_cost)"},
                    {"po_vend_num","po_vend_num"},
                },
        ...
        public (
            ...
            int? ReturnCode)
        AltExtGen_Home_MetricVendorCommittedSp(
            string AltExtGenSp,
            int? Count = 10)
        {
