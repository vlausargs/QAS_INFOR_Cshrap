...
{
    [IDOExtensionClass("SLAptrxpAlls")]
    public class SLAptrxpAlls : CSIExtensionClassBase
    {
		                                    ...
		                                    ref string Infobar)
		{
			using(var MGAppDB = this.CreateAppDB())
			{
				...
				var iDefAPPostedTransFilterExt = new DefAPPostedTransFilterFactory().Create(appDb);
				
				int Severity = iDefAPPostedTransFilterExt.DefAPPostedTransFilterSp(ref SiteFilterVar,
				                                                                   ref Infobar);
        ...
        [IDOMethod(MethodFlags.None, "Infobar")]
        public int TwoCurrCnvtSp(string FromCurrCode,
                                 byte? UseBuyRate,
                                 byte? RoundResult,
                                 [Optional] DateTime? Date,
                                 [Optional] byte? RoundPlaces,
                                 [Optional, DefaultParameterValue((byte)0)] byte? UseCustomsAndExciseRates,
        [Optional] byte? ForceRate,
        [Optional] byte? FindTTFixed,
        [Optional] ref decimal? TRate,
        ref string Infobar,
        [Optional] string Site,
        string ToCurrCode,
        [Optional] ref byte? TRateD,
        [Optional, DefaultParameterValue((byte)0)] byte? InvertExchRate,
        decimal? Amount1,
        ref decimal? Result1,
        ...
        [Optional] ref decimal? Result15)
        {
            using (var MGAppDB = this.CreateAppDB())
            {
                ...
                var i2CurrCnvtExt = new TwoCurrCnvtFactory().Create(appDb);

                var result = i2CurrCnvtExt.TwoCurrCnvtSp(FromCurrCode,
                                                       UseBuyRate,
                                                       RoundResult,
                                                       Date,
                                                       RoundPlaces,
		...
		[IDOMethod(MethodFlags.None, "Infobar")]
		public int ApplyQuickPaymentsPostUpdSp(Guid? SessionId)
		{
			using(var MGAppDB = this.CreateAppDB())
			{
				var appDb = new CSIAppDBFactory().CreateAppDB(MGAppDB, this.Context, this.GetMessageProvider());
				...
				var iApplyQuickPaymentsPostUpdExt = new ApplyQuickPaymentsPostUpdFactory().Create(appDb, bunchedLoadCollection);
		...
		public int TmpApQuickPaymentsCleanUpSp(Guid? SessionId)
		{
			using(var MGAppDB = this.CreateAppDB())
			{
				...
				var iTmpApQuickPaymentsCleanUpExt = new TmpApQuickPaymentsCleanUpFactory().Create(appDb);
				
				var result = iTmpApQuickPaymentsCleanUpExt.TmpApQuickPaymentsCleanUpSp(SessionId);
		...
		[IDOMethod(MethodFlags.CustomLoad, "Infobar")]
		public DataTable CLM_AllDueSp(string pBankCode,
		string pVendNum,
		int? pCheckSeq,
		DateTime? pCheckDate,
		int? pCheckNum,
		[Optional] string FilterString)
		{
			using(var MGAppDB = this.CreateAppDB())
			{
				...
				var iCLM_AllDueExt = new CLM_AllDueFactory().Create(appDb,
				bunchedLoadCollection,
				mgInvoker,
				new CSI.Data.SQL.SQLParameterProvider(),
				true);
				
				var result = iCLM_AllDueExt.CLM_AllDueSp(pBankCode,
				pVendNum,
				pCheckSeq,
				pCheckDate,
				pCheckNum,
