...
{
	[IDOExtensionClass("SLEdiPoAckItems")]
	public class SLEdiPoAckItems : CSIExtensionClassBase
	{
		[IDOMethod(MethodFlags.CustomLoad, "Infobar")]
		public DataTable EdiPoAckValidateSp(string PoNum,
		int? SeqNum,
		string VendNum,
		[Optional, DefaultParameterValue(0)] int? TPost,
		ref string Infobar,
		[Optional] string FilterString)
		{
			using(var MGAppDB = this.CreateAppDB())
			{
				...
				var iEdiPoAckValidateExt = new EdiPoAckValidateFactory().Create(appDb,
				bunchedLoadCollection,
				mgInvoker,
				new CSI.Data.SQL.SQLParameterProvider(),
				true);
				
				var result = iEdiPoAckValidateExt.EdiPoAckValidateSp(PoNum,
				SeqNum,
				VendNum,
				TPost,
				Infobar,
