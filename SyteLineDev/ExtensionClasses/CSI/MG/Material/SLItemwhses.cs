...
{
    [IDOExtensionClass("SLItemwhses")]
    public class SLItemwhses : CSIExtensionClassBase, IExtFTSLItemwhses, ISLItemwhses, CSI.ExternalContracts.Portals.ISLItemWhses
    {
        public override void SetContext(IIDOExtensionClassContext context)
        {
            base.SetContext(context);

            this.Context.IDO.PostUpdateCollection += HandlePostUpdateCollection;
            this.Context.IDO.PreUpdateCollection += HandlePreUpdateCollection;
        }

        private ITriggerState triggerState = new CSIExtensionClassState();
        ...
        private void HandlePreUpdateCollection(object sender, IDOEventArgs args)
        {
            ...
            {
                var factory = new MGPostUpdateCollectionHandlerFactory();
                var handler = factory.Create(this.Context, MGAppDB);
                this.triggerState = handler.PrepareForTrigger(
                    triggerState,
                    (args as IDOUpdateEventArgs).ActionMask,
                    (UpdateCollectionRequestData)args.RequestPayload,
                    (UpdateCollectionResponseData)args.ResponsePayload);
            }
        }

        [IDOAddEventHandler(IDOEvent.PostUpdateCollection)]
        ...
        [IDOMethod(MethodFlags.None, "Infobar")]
        public int ItemwhseWithNameValidSp(ref string Whse,
                                                   string Item,
                                                   ref string Name,
                                                   ref string Infobar)
        {
            using (var MGAppDB = this.CreateAppDB())
            {
                ...
                var iItemwhseWithNameValidExt = new ItemwhseWithNameValidFactory().Create(appDb);

                WhseType oWhse = Whse;
                NameType oName = Name;
                InfobarType oInfobar = Infobar;

                int Severity = iItemwhseWithNameValidExt.ItemwhseWithNameValidSp(ref oWhse,
                                                                       Item,
                                                                       ref oName,
                                                                       ref oInfobar);
        ...
        [IDOMethod(MethodFlags.None, "Infobar")]
        public int ValidateItemForReplenPOBlnSp(string PPONum,
                                                        string PItem,
                                                        ref string Infobar)
        {
            using (var MGAppDB = this.CreateAppDB())
            {
                ...
                var iValidateItemForReplenPOBlnExt = new ValidateItemForReplenPOBlnFactory().Create(appDb);

                InfobarType oInfobar = Infobar;

                int Severity = iValidateItemForReplenPOBlnExt.ValidateItemForReplenPOBlnSp(PPONum,
                                                                            PItem,
                                                                            ref oInfobar);
		...
		[IDOMethod(MethodFlags.None, "Infobar")]
		public int CheckConsgnWhseForItemSp(string Item,
			string Whse,
			ref string Infobar)
		{
			var iCheckConsgnWhseForItemExt = this.GetService<ICheckConsgnWhseForItem>();
			
			var result = iCheckConsgnWhseForItemExt.CheckConsgnWhseForItemSp(Item,
				Whse,
				Infobar);
		                                           ...
		                                           ref string Infobar)
		{
			using(var MGAppDB = this.CreateAppDB())
			{
				...
				var iGetVendConsignmentUsageByItemExt = new GetVendConsignmentUsageByItemFactory().Create(appDb);
				
				int Severity = iGetVendConsignmentUsageByItemExt.GetVendConsignmentUsageByItemSp(pVendNum,
				                                                                                 pItem,
				                                                                                 pStartDate,
				                                                                                 pEndDate,
				                                                                                 ref BeginBalance,
		...
		[IDOMethod(MethodFlags.None, "Infobar")]
		public int WhseQtyListSp([Optional, DefaultParameterValue(null)] string PWhse,
		string PIitem,
		ref decimal? PTotalOnHand,
		ref string PWhseOnHand)
		{
			using(var MGAppDB = this.CreateAppDB())
			{
				var appDb = new CSIAppDBFactory().CreateAppDB(MGAppDB, this.Context, this.GetMessageProvider());
				
				var iWhseQtyListExt = new WhseQtyListFactory().Create(appDb);
				
				var result = iWhseQtyListExt.WhseQtyListSp(PWhse,
				PIitem,
				PTotalOnHand,
				PWhseOnHand);
				
				int Severity = result.ReturnCode.Value;
				PTotalOnHand = result.PTotalOnHand;
				PWhseOnHand = result.PWhseOnHand;
				return Severity;
			}
		}
