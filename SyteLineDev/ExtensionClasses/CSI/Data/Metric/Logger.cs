...
namespace Metric.Logger
{
    public class Logger : NLog.ILogger
    {
        private readonly CSI.MG.ILogger _logger;
        private readonly string sourceName = "Metric";
        public Logger(CSI.MG.ILogger logger)
        {
            _logger = logger;
        }
        public void Log(LogLevel level, [Localizable(false)] string message)
        {
            if (level == LogLevel.Debug) _logger.Debug(sourceName, message);
            else if (level == LogLevel.Error) _logger.Error(sourceName, message);
            else if (level == LogLevel.Info) _logger.Info(sourceName, message);
            else if (level == LogLevel.Trace) _logger.Trace(sourceName, message);
            else if (level == LogLevel.Warn) _logger.Warn(sourceName, message);
            else if (level == LogLevel.Fatal) _logger.Fatal(sourceName, message);
        }

        #region Not used on Metrics. Those methods are ignored without implementation.
