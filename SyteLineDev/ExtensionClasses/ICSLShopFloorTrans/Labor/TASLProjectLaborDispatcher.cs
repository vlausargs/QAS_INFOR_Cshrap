...
{
    public class TASLProjectLaborDispatcher : ALaborUtils, DispatcherService
    {

        public TASLProjectLaborDispatcher(IIDOExtensionClassContext context)
        {
        ...
        private void AddToPostJob(InputDataSet.HoursLine hrsLine)
        {
            ...
            string totalTimeInMints = CleanString(hrsLine.GetValue("ManTimeInMins"));
        ...
        private bool deleteDataForMaster(string employee, InputDataSet.MasterLine masterLine, bool postOffSets)
        {
            ...
            /* We are intentionally NOT deleting unprocessed records for TASL types. We're 
             * only posting new records through this dispatcher.
             */
            //deleteUnProcessedRecords(employee, reportDate);

            /* We are intentionally NOT using the offset routine for TASL types. We're 
             * calculating/constructing the offset record in FT and sending it over to 
             * this TASL method to be posted as a new record.
             */
        ...
        private bool offsetPostedRecords(string employee, string reportDate,bool postOffSets)
        {
            ...
            string filterString = "EmpNum = '" + employee + "' and TranDate='" + reportDateTime.ToString("yyyy/MM/dd") + "' and TranType='L'";
        ...
        /// <summary>
        /// Ran into an issue where parsing the mantimeinmin value to double in AddToPostJob would throw the 
        /// error "Input string was not in a correct format". Trimming and Normalizing the string didn't work.
        /// Returning a brand new string from StringBuilder appears to remove some encoding issue in the
        /// raw value from the XML line that got into the string snippet that is being returned.
        /// The use of new StringBuilder below might need to be added to InputDataSet's GetValue function, so 
        /// that all GetValue calls will receive a new, cleansed string, to address encoding issues anywhere 
        /// else in the system as well.
        /// </summary>
        /// <param name="input">String to be cleaned</param>
        /// <returns>Brand new string representation of the input string</returns>
