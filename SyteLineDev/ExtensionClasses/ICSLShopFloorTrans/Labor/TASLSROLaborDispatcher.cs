...
{
    public class TASLSROLaborDispatcher : ALaborUtils, DispatcherService
    {
        private List<string> partnerIDs = null;

        #region Dispatcher Methods
        public TASLSROLaborDispatcher(IIDOExtensionClassContext context)
        {
        ...
        {
            IDORuntime.LogUserMessage("TASLSROLaborDispatcher.offsetPostedRecords", UserDefinedMessageType.UserDefined1, "inside offset posting");
            ...
            DateTime dbDefaultDate = DateTime.ParseExact("01011900 0000", "MMddyyyy HHmm", CultureInfo.CurrentCulture);

            LoadCollectionResponseData responseData1 = null;
            LoadCollectionResponseData responseData2 = null;
            LoadCollectionResponseData responseData3 = null;

            string propertyList = "SroNum, SroLine, SroOper, StartDate, EndDate, HrsWorked, HrsToBill, WorkCode, " +
                "BillCode, TransNum, RowPointer, RecordDate";

            IDORuntime.LogUserMessage("TASLSROLaborDispatcher.offsetPostedRecords", UserDefinedMessageType.UserDefined1, "inside offset posting 2");
            ...
            if (reportDateStart.Equals(reportDateEnd))
            {
                ...
                requestData1.Filter = "PartnerId = '" + partnerID + "'";
                ...
                requestData1.Filter += " and StartDate >= '" + reportStartDateTime.ToString(WMShortDatePattern) + " " + reportStartDateTime.ToString(WMTimePattern) + "'";
                requestData1.Filter += " and EndDate <= '" + reportEndDateTime.ToString(WMShortDatePattern) + " " + reportEndDateTime.ToString(WMTimePattern) + "'";
            ...
            else
            {
                ...
                requestData1.Filter += " and TransDate ='" + reportStartDateTime.ToString(WMShortDatePattern) + "'"; //MSF157397 Date time conversion issue when the customer has culture settings other than US JH:20130128
                requestData1.Filter += " and StartDate >= '" + reportStartDateTime.ToString(WMShortDatePattern) + " " + reportStartDateTime.ToString(WMTimePattern) + "'";
                requestData1.Filter += " and EndDate <= '" + reportEndDateTime.ToString(WMShortDatePattern) + " " + etInSeconds.ToString(WMTimePattern) + "'";
                requestData1.RecordCap = 20000; // -1;
                requestData1.OrderBy = "TransNum";
                responseData1 = this.Context.Commands.LoadCollection(requestData1);
                ...
                requestData2.Filter = "PartnerId = '" + partnerID + "'";
                ...
                requestData2.Filter += " and StartDate >= '" + reportEndDateTime.ToString(WMShortDatePattern) + " " + stInSeconds.ToString(WMTimePattern) + "'";
                requestData2.Filter += " and EndDate <= '" + reportEndDateTime.ToString(WMShortDatePattern) + " " + reportEndDateTime.ToString(WMTimePattern) + "'";
            ...
            requestData3.IDOName = "FSSROLabors";
            requestData3.PropertyList.SetProperties(propertyList);
            requestData3.Filter = "PartnerId = '" + partnerID + "'";
            ...
            requestData3.Filter += " and '" + reportStartDateTime.ToString(WMShortDatePattern) + " " + reportStartDateTime.ToString(WMTimePattern) + "' between StartDate and EndDate";
            ...
            {
                /* modify the following response data:
                 * set StartDate to the adjusted clock out time
                 * compute new total hours from new start to existing end time
                 *      Update hrs_worked
                 *      Update hrs_to_bill if it's <> 0
                ...
                */
                for (int i = 0; i < responseData3.Items.Count; i++)
                {
                    ...
                    try
                    {
                        dtStartDate = DateTime.ParseExact(responseData3[i, "StartDate"].Value, "yyyyMMdd HH:mm:ss.fff", CultureInfo.CurrentCulture);
                        dtEndDate = DateTime.ParseExact(responseData3[i, "EndDate"].Value, "yyyyMMdd HH:mm:ss.fff", CultureInfo.CurrentCulture);
                    }
                    catch (Exception e)
                    {
                        IDORuntime.LogUserMessage("TASLSROLaborDispatcher.offsetPostedRecords", UserDefinedMessageType.UserDefined1, "Error [" + e.Message + "]");
                    }
                    if (!dtStartDate.Equals(DateTime.MinValue) && !dtEndDate.Equals(DateTime.MinValue))
                    {
                        DateTime adjustedStartDate = dtStartDate.Date.Add(reportStartDateTime.TimeOfDay); // combine the special database populated date (1900-01-01) in StartDate and the newly computed start time
                        TimeSpan this_elapsed = dtEndDate.Subtract(adjustedStartDate);

                        responseData3[i, "StartDate"].Value = adjustedStartDate.ToString("yyyyMMdd HH:mm:ss.fff");
                        responseData3[i, "HrsWorked"].Value = this_elapsed.TotalHours.ToString();

                        // only update hrsToBill if it's not empty or equal to zero
                        if (!string.IsNullOrEmpty(responseData3[i, "HrsToBill"].Value))
                        {
                            if (Convert.ToDouble(responseData3[i, "HrsToBill"].Value, CultureInfo.InvariantCulture) > 0) // FTDEV-9247
                            {
                                responseData3[i, "HrsToBill"].Value = this_elapsed.TotalHours.ToString();
                            }
                        }
            ...
            for (int i = 0; i < responseData1.Items.Count; i++)
            {
                // "SroNum, SroLine, SroOper, StartDate, EndDate, HrsWorked, HrsToBill, WorkCode, BillCode, TransNum, RowPointer, RecordDate"
                try
                {
                    ...
                    try
                    {
                        startDate = DateTime.ParseExact(startDate, "yyyyMMdd HH:mm:ss.fff", CultureInfo.CurrentCulture).ToString("yyyyMMdd HH:mm:ss.fff");
                        endDate = DateTime.ParseExact(endDate, "yyyyMMdd HH:mm:ss.fff", CultureInfo.CurrentCulture).ToString("yyyyMMdd HH:mm:ss.fff");
                    }
                    catch (Exception e)
                    {
                        IDORuntime.LogUserMessage("TASLSROLaborDispatcher.offsetPostedRecords", UserDefinedMessageType.UserDefined1, "Error building offset record [" + e.Message + "]");
                    }
        ...
        private LoadCollectionResponseData RemoveDuplicates(LoadCollectionResponseData responseData)
        {
            ...
            for (int i = 0; i < responseData.Items.Count; i++)
            {
                 ...
                 * 
             * 
             * SroNum, SroLine, SroOper, StartDate, EndDate, HrsWorked," +
        ...
        private void AddToPostJob(InputDataSet.HoursLine hrsLine)
        {
            ...
            jobInfo.hoursWorked = caltotalTimeInHours.ToString("0.00000000");
            jobInfo.billHours = caltotalTimeInHours.ToString("0.00000000");
